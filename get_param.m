...
for i = 1:N
    for j = 1:T
        mixtures(i, j) = mixture(mix(i), O(j, :));
    end
end
% 给定观察序列O, 计算前向概率alpha
alpha = zeros(T,N);
...
for t = T-1:-1:1
	...
	for i = 1:N
	for j = 1:N
		beta(t,i) = beta(t,i) + beta(t+1,j) * mixtures(j, t+1) * trans(i,j);
	end
	end
	beta(t,:) = c(t) * beta(t,:);
end
...
for t = 1:T-1
	...
	for i = 1:N-1
	for j = i:i+1
		nom = alpha(t,i) * trans(i,j) * mixtures(j, t+1) * beta(t+1,j);
